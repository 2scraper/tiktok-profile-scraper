"""
tiktok-profile-scraper — 2captcha Scraper API edition (fourth engine)
...
What this path does HERE: a TikTok account's public statistics.

A TikTok profile page is server-rendered whole — the account object is in
the document the service returns — so this client produces the same row as
the three engines. Nothing on the row is reachable only from a browser.

And the honest sentence about when to pay for this at all: on this route,
not for access. Measured 2026-09-22 from a bare datacentre address with no
key and no proxy, the profile page was served to plain curl. What this
client buys is somebody else's infrastructure and a specific exit.
...
from product_parser import (STATE_CHALLENGE, STATE_CONTENT,
                            STATE_USER_UNAVAILABLE, challenge_markers_present,
                            detect_page_state, handle_from_url,
                            normalise_handle, parse_profile, profile_url)
from tiktok_payload import PayloadError
from output_writer import (EXIT_NO_PRODUCTS, Profile, finish_run, utc_now)
import env_config
...
def _run_once(args, attempt: int = 1, attempts: int = 1) -> int:
    ...
    if state == STATE_USER_UNAVAILABLE:
        ...
        logger.error(
            "TikTok returned no account for %s. It reports the same status "
            "for a banned account and for one that never existed, so this "
            "is one or the other — it is not a block, and routing through "
            "another exit will not change it.", args.url)
        return EXIT_NO_PRODUCTS

    try:
        rows, diag = parse_profile(html, args.url, utc_now(), Profile)
        ...
        logger.error(
            ...
            "%s — saved to %s. A served TikTok profile page always has one, "
            "so this is either a page kind this client does not read or a "
            "change in the site. NOT reported as 'no account': a served "
    ...
    if row is None or not row.sku:
        ...
        with open(dump, "w", encoding="utf-8") as handle:
            handle.write(html)
        logger.error("The payload was present but no profile row came out "
    ...
    logger.info("Read @%s (%s) — %s followers, %s likes, %s videos, read "
                "from %s. This path produces the same columns as the three "
                "engines: a TikTok profile page is server-rendered whole, "
                "so nothing on this row is reachable only from a browser.",
                row.username, row.sku,
                f"{row.follower_count:,}" if row.follower_count is not None else "?",
                f"{row.likes_count:,}" if row.likes_count is not None else "?",
                row.video_count if row.video_count is not None else "?",
                row.stats_source)
    if row.stats_source != "statsV2":
        logger.warning("This row's counts are ROUNDED: TikTok published no "
                       "statsV2 for it, and the `stats` object it did "
                       "publish rounds to three significant figures — by up "
                       "to 43,287 followers on the accounts measured.")
    if row.is_seller:
        logger.info("This account is a TikTok Shop seller — see "
                    "tiktok-shop-scraper.")
...
def parse_args():
    p = argparse.ArgumentParser(
        description="tiktok-profile-scraper — 2captcha Scraper API edition (no local "
                    "browser). Reads a TikTok account's public statistics through the service. See "
    ...
    p.add_argument("--url", default=None,
                   help="A TikTok handle ('nasa', '@nasa') or a profile URL. Required, unless TIKTOK_URL is "
                        "set in the environment or .env.")
    p.add_argument("--mode", choices=("profile",), default="profile",
                   help="The one mode this path serves on this route.")
    p.add_argument("--category", default=None,
                   help="Label to tag the run with in the sidecar.")
    p.add_argument("--format", choices=["json", "csv", "both"], default="both")
    p.add_argument("--out", default="tiktok_profile_scraperapi",
