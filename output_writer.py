...
This repo reads one KIND of thing, so it carries one dataclass. Its
siblings — tiktok-video-scraper, tiktok-shop-scraper and
tiktok-ads-scraper — read different kinds and carry their own, and all of
them keep the family prefix
...
class Profile:
    ...
    (CLAUDE.md §9). `sku` is the account's NUMERIC id, not its handle:
    TikTok lets a user change the handle and publishes
    `uniqueIdModifyTime` to say when they last did, so a diff joined on the
    handle would report one renamed account as one deletion plus one
    arrival. The handle is carried in `username`, which is what a human
    searches for.

    `title` is the display name (`nickname`), which is the account's human
    label and the thing a CSV of ten thousand accounts is read by.
    """

    source: str = SOURCE_DEFAULT
    scraped_at: str = ""
    url: Optional[str] = None
    # The permanent numeric account id.
    sku: Optional[str] = None
    # The display name.
    title: Optional[str] = None

    # ---- identity ---------------------------------------------------------
    # The @handle, without the @. Changeable by the user — see the class
    # docstring for why it is not the key.
    username: Optional[str] = None
    # The same value as `sku`, under the name the rest of TikTok's own API
    # uses for it. Carried so a consumer joining against TikTok's payloads
    # does not have to know that this family calls every id `sku`.
    user_id: Optional[str] = None
    # TikTok's opaque per-account token. Required as an input by several of
    # TikTok's own endpoints, so a row is directly usable as the input to
    # further work rather than needing the profile fetched again.
    sec_uid: Optional[str] = None

    # ---- counts -----------------------------------------------------------
    # EXACT, from `statsV2`. The neighbouring `stats` object publishes the
    # same figures rounded to three significant figures — 95,900,000 where
    # statsV2 says 95,856,713 — and it rounds both up and down, so it
    # cannot be corrected for. `stats_source` records which object a row
    # was actually read from; see tiktok_payload.counts().
    follower_count: Optional[int] = None
    following_count: Optional[int] = None
    # Total likes the account has received across its videos ("hearts").
    likes_count: Optional[int] = None
    video_count: Optional[int] = None
    # Accounts that follow this one and are followed back.
    friend_count: Optional[int] = None
    # Likes this account has GIVEN. Publicly zero on most accounts, because
    # TikTok lets a user hide their liked videos and reports 0 when hidden
    # — so a 0 here means "0 or hidden", which is why `open_favorite` is
    # carried beside it.
    digg_count: Optional[int] = None
    # likes_count / video_count, computed from two numbers the site
    # published. Null rather than 0 when the account has no videos.
    likes_per_video: Optional[float] = None
    # "statsV2" | "stats" | "absent" — provenance, per CLAUDE.md §8. A diff
    # whose count difference comes with a stats_source difference says
    # something about our two snapshots, not about the account.
    ...
    # TikTok Shop seller. The single field linking this repo to
    # tiktok-shop-scraper, which is why it is carried here despite being
    # false on most accounts.
    is_seller: Optional[bool] = None
    commerce_user: Optional[bool] = None
    secret: Optional[bool] = None

    # ---- what the account says about itself --------------------------------
    bio: Optional[str] = None
    # The external link in the bio. Sparse by nature — TikTok grants it to
    # some account types only, and it was set on 2 of 14 profiles measured
    # on 2026-09-22 — but it is the highest-value field on the row for
    # anyone doing outreach, so a sparse column is the right call here
    # rather than the §9 case for dropping one.
    bio_link: Optional[str] = None
    # The avatar is served from a SIGNED CDN URL that expires. Measured
    # 2026-09-22: the signed form answers HTTP 200 with a 77 KB JPEG and
    # carries `x-expires` 47.7 hours out; the same URL with the query
    # string removed answers HTTP 403. So the link is genuinely usable and
    # genuinely perishable, and both halves have to reach the consumer.
    avatar_url: Optional[str] = None
    # The stable part: the content hash in the CDN path, which does not
    # change between fetches and DOES change when the account changes its
    # picture. Without it, `avatar_url` differs on every single row of
    # every single run — the signature is re-minted per request — and
    # `diff_runs.py` would report every account as changed, every time.
    # CLAUDE.md §24 records the same shape as a float32 round-trip making
    # two modes disagree on every row; this is that, with a signature.
    avatar_id: Optional[str] = None
    # When `avatar_url` stops working, from the URL's own `x-expires`.
    avatar_expires_at: Optional[str] = None
    language: Optional[str] = None

    # ---- when -------------------------------------------------------------
    # ISO-8601 UTC. TikTok publishes these as unix seconds and uses 0 for
    # "never", which is recovered to null rather than to 1970.
    created_at: Optional[str] = None
    # Set on 3 of 12 accounts measured 2026-09-22 — a real, sparse column.
    nickname_modified_at: Optional[str] = None
    #
    # `username_modified_at` was here and is GONE. TikTok publishes
    # `uniqueIdModifyTime` on every account and reported 0 — "never" — on
    # ALL EIGHTEEN measured on 2026-09-22 (six fixtures plus twelve large
    # public accounts including several that have demonstrably renamed).
    # CLAUDE.md §9: a column null on every row of every run should not
    # exist, and removing it needs the measurement written down so somebody
    # can add it back with a better one. This is that measurement.

    # ---- settings the account publishes -------------------------------------
    # TikTok's own small integers, passed through unchanged rather than
    # mapped to words this repo would have to invent. 0 is "everyone" on
    # every one of them in the values measured, but the mapping is not
    # documented by TikTok and guessing at it would be presenting a guess
    # as a fact (CLAUDE.md §8).
    comment_setting: Optional[int] = None
    duet_setting: Optional[int] = None
    stitch_setting: Optional[int] = None
    download_setting: Optional[int] = None
    following_visibility: Optional[int] = None
    open_favorite: Optional[bool] = None
    show_playlist_tab: Optional[bool] = None

    # ---- what the site said about the request -------------------------------
    # "ok" on every emitted row. The column exists because the OTHER value
    # is the interesting one and it reaches the sidecar rather than a row:
    # TikTok answers a handle that does not exist with statusCode 10221 and
    # the message "user banned", so nonexistent and banned are ONE state
    # here and this repo does not claim to tell them apart.
    status: Optional[str] = None
    status_code: Optional[int] = None
    #
    # `status_msg` was here and is GONE, for a structural reason rather
    # than a measured one: it is only ever set when TikTok returns NO
    # account, and a run with no account emits no row — so the column could
    # not be non-null on an emitted row under any circumstances. It reaches
    # the reader through the sidecar's `handles_unavailable` instead, which
    # is where a fact about a handle that produced no row belongs.
...
# place.
ROW_CLASS_BY_MODE = {"profile": Profile}
...
def dedupe_by_key(rows: Sequence[Any], seen: Set[str], key: str = "sku") -> List[Any]:
    ...
    means the same account was asked for twice in one run, which is worth
